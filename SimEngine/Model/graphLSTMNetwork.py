...
class graphLSTMNetwork(nn.Module):
    ...
    def __init__(self,id2vec,api2vec,embedding_dim,hidden_dim,num_layers,max_nodes,batch_size,out_dim,with_kb):
        super(graphLSTMNetwork,self).__init__()
        ...
        self.id2vec = id2vec
        # self.kb2vec = api2vec
        self.i_encoder = nn.GRU(input_size= self.embedding_dim,
    ...
    def forward(self,inputs):
        ...
        #                   g1_api, g2_api, g1_api_length, g2_api_length]
        ...
        if self.with_kb:
            ...
            a_api_out, b_api_out = self.get_kb_embed(inputs[0],inputs[7],inputs[3],inputs[8])
        ...
        # lable = inputs[5]
        # pairs = [inputs[0].to(DEVICE), a_embeds, inputs[2].to(DEVICE),inputs[3].to(DEVICE), b_embeds, inputs[4].to(DEVICE)]
        # pairs = [inputs[0], a_embeds, inputs[2], inputs[3], b_embeds,inputs[4]]
        # predictions= self.get_lstm_similarity(pairs)
        # predictions = self.get_attention_similarity(pairs)
        # return  predictions
    ...
    def get_kb_embed(self,g1_adj, a_kb,g2_adj,b_kb):
