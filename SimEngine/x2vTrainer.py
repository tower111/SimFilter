...
class x2vTrainer(object):
    ...
    def test(self):
        ...
        if test_auc>self.BEST_TEST_AUC:
            ...
            fig.savefig(config.trainResult+'/comFU_W_new'+'_'+self.network+'_'+self.cat_kb+'.png')
            print("\nNEW BEST_VAL_AUC: {} !\n\ttest_auc : {}\n".format(self.BEST_VAL_AUC, test_auc))
            np.savez(config.trainResult+'/comFU_W_new' + '_' + self.network + '_' + self.cat_kb + '.result', fpr=test_fpr,
        ...
        print(log_string)
        self.log(config.trainResult+'/LSTM.log', log_string)
...
if __name__ == '__main__':
    ...
    #-i data/w2v/word2id.json -v data/w2v/embedding_matrix.npy -x data/datacenter/train_dataset.json -y data/datacenter/valid_dataset.json
    # -z data/datacenter/test_dataset.json -n seq_SAFE -k False
