...
class pairsDataset(object):
    ...
    def get_seq_from_cfg(self,cfg):
        ...
        return a_ids,[],a_length ,[]#a_api_ids ,a_api_length

    def getFeatures(self,cfg,raw_apis):
        constant = 0
        string = 0
        num_transfer = 0
        num_calls = 0
        ...
        apis = raw_apis
        for node in cfg:
            constant = constant + node['features']['constant']
            string =  string + node['features']['string']
            num_transfer = num_transfer + node['features']['transfer']
            num_calls = num_calls + node['features']['call']
            num_arith = num_arith + node['features']['arith']

        return [constant,string,num_transfer,num_calls,num_arith,apis]
    ...
    def get_graph_pairs(self,batch_data):
