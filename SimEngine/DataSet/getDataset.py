...
class getDataset(object):
        ...
        with open(file_a, 'r') as afile:
            with open(file_b, 'r') as bfile:
                ...
                for funa in a_functions:
                    if file_a_json['filename'] != 'UNKname':
                        a_name = funa['name']    #文件名到这里已经相等了
                        # a_file = funa['file']
                        cfga = a_name

                        for funb in b_functions:
                            if file_b_json['filename'] != 'UNKname':
                                b_name = funb['name']
                                # b_file = funb['file']
                                cfgb =  b_name
                                true_pair = dict()
                                if cfga == cfgb:
                                    ...
                                    if self.min_node<= lena <= self.max_node and\
                                            self.min_node<= lenb <= self.max_node :
                                        ...
                                        true_pair['filea'] = file_a_json['filename']
                                        true_pair['fileb'] = file_b_json['filename']
                            ...
                            if file_b_json['filename'] != 'UNKname':
                                if cfga != cfgb:
                                    ...
                                    if self.min_node<= lena <= self.max_node and self.min_node<= lenb <= self.max_node:
                                        ...
                                        false_pair['filea'] = file_a_json['filename']
    ...
    def get_dataset(self):
        ...
        for dataset in comp_files:
            ...
            file_a_name = file_a.split('_')[-1].split('.')[0] #dappp
...
if __name__ == '__main__':
    ...
    #-i /home/yuqing/Desktop/10t-net/tower/PaperProject/SimFilter/data/binJson -d /home/yuqing/Desktop/10t-net/tower/PaperProject/SimFilter/data/datacenter -min 1 -max 1000
    # # min max node nums
    # min_node = 1
    # max_node = 1000
    # train:valid:test
